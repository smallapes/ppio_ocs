...
5、转人工：用户输入的问题非常明确，但是既不准确属于智能调用的内容，也不是要诊断跑量低，也不能在问题集中找到现成答案，则需要人工服务，则用户需求属于转人工。
...
# 判定为智能问答、跑量诊断、智能调用、不清楚必须谨慎，除非你确定，否则一定不要判定为智能问答、跑量诊断、智能调用、不清楚!
# 判定为智能问答、跑量诊断、智能调用必须谨慎，除非你确定，否则不要判定为智能问答、跑量诊断、智能调用! 
...
3、是看监控跑量图，看最近7天跑量，看波形是否异常，是否有异常时段，如果有检查该时段是否有丢包延迟，如果有询问应商是否有操作，协助定位原因
4、如果网络没问题，就看磁盘是否有过高延迟，看硬件cpu负载是否过高，如果过高则硬件有问题，告诉供应商整改。
5、如果上面诊断不出原因，可能是程序的问题(例如，docker掉了，系统跑坏了)，联系运维人员用跳板机远程上机器查看。
