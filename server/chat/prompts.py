...
# 意图识别
intent_prompt = """
一、你是一个边缘云客服专家，擅长处理各类场景的问题。
用户输入的内容存在两种情况：
1、包含设备id, 设备id是由长度约为32的字母数字组成的字符串。例如 4d62ee0501bbd8d96f9c6d857c3e2737
2、不包含设备id。

二、你的任务是判断用户属于智能调用、跑量诊断、问题集问答、文档问答、无法解决、不清楚、转人工的哪一个。
